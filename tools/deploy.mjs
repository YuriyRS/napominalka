...
/* Знаки валют проверяются отдельно, и это не придирка: шрифт разбит
   на подмножества по диапазонам символов, и если знака в них нет, браузер
   молча подставит системный шрифт ровно на этом знаке — посреди строки,
   в одном месте приложения. Заметить это можно только на экране подписок,
   да и то если знать, куда смотреть. */
say('знаки валют', await b.evalIn(
  `document.fonts.ready.then(() => ['₽', '₸', '$'].map((s) =>
     s + (document.fonts.check('16px "Golos Text"', s) ? ' есть' : ' НЕТ')).join(', '))`,
));
