...
made.push(write('5. Сборка для магазина/ГДЕ ВЗЯТЬ.txt',
  `Подписанная сборка лежит здесь:
https://github.com/YuriyRS/napominalka/releases/download/rustore/domovoy-release.apk

Ссылку открывайте на том компьютере, с которого будете загружать файл
в консоль RuStore. Файл весит около 3,5 МБ.

Обновляется сам: любое изменение в проекте запускает сборку, она
подписывает новую версию и кладёт её по этой же ссылке.

ЧЕГО НЕ БРАТЬ
Файл из релиза apk (domovoy.apk, 4,4 МБ) — это сборка для телефона.
Она подписана отладочным ключом, и магазин её не примет.`));
