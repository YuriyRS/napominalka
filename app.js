...
/* Просили ли мы систему поставить виджет в этот запуск.

   Нужно, чтобы отличить два разных случая, которые выглядят одинаково:
   «человек ещё не пробовал» и «пробовал, система не поставила». Без этой
   заметки строка после неудачи показывала бы то же самое, что и до неё,
   и человек решал бы, что нажатие не сработало. */
...
async function syncWidgetRow() {
  ...
  if (placed) {
    widgetHint.textContent = 'Если он не нужен: долгое нажатие на плитку → «Убрать с экрана»';
    widgetAsked = false;
    return;
  }

  /* Системный вопрос был, человек согласился — а плитки нет. Так бывает:
     диалог показывают многие оболочки, а кладут виджет не все. Говорим
     об этом прямо и ведём в обходной путь, вместо того чтобы оставить
     человека гадать, куда делось подтверждение. */
  widgetHint.textContent = widgetAsked
    ? 'Система не поставила. Вручную: долгое нажатие на пустом месте '
      + 'рабочего стола → «Виджеты» → «Домовой»'
...
widgetBtn?.addEventListener('click', async () => {
  ...
  if (!res.ok) {
    widgetHint.textContent = `Поставить вручную: долгое нажатие на пустом месте `
      + `рабочего стола → «Виджеты» → «Домовой»`;
    return;
  }
  /* Система показала свой вопрос. Ответ придёт не сразу, а когда человек
     на него ответит, — и не факт, что он вообще что-то поставит. Поэтому
     запоминаем, что просили: при возвращении проверим и скажем. */
  widgetAsked = true;
...
async function syncAlarms() {
  ...
  /* Строка в настройках показывает то, что стоит у системы, — значит,
     обновлять её надо после того, как там что-то поменялось. Здесь,
     а не в render(): render бывает и без единого изменения в будильниках,
     и спрашивать у системы каждый раз было бы незачем. */
...
async function syncRemindersRow() {
  ...
  if (n) {
    ...
    if (!on) {
      hint.textContent = perm === 'granted' ? 'Сейчас выключены — нажмите, чтобы включить'
        : perm === 'prompt' ? 'Разрешите уведомления — иначе напоминать нечем'
        : 'Уведомления запрещены — разрешите их в настройках телефона';
      return;
    }

    /* А когда включены — говорим, сколько их у системы и когда ближайшее.
       Это единственное место, где видно разницу между «приложение
       не поставило будильник» и «поставило, а система задержала»:
       без него и то и другое выглядит одинаково — напоминание не пришло. */
    const p = await n.pending();
    hint.textContent = p.count
      ? `Назначено: ${p.count}. Ближайшее — ${p.at ? leftLabel(p.at) : 'без времени'}`
      : 'Пока нечего напоминать — заведите дело со временем';
    return;
  }
...
async function exportBackup() {
  ...
  a.href = url;
  a.download = `домовой-${new Date().toISOString().slice(0, 10)}.json`;
...
async function refresh() {
  ...
  // ради чего стоит задерживать отрисовку, к тому же это поход в сеть.
  syncTimes();

  /* А будильники в приложении — ждём.

     Разница не в аккуратности, а в цене ошибки. Задача, поставленная
     на 15:03 и сохранившаяся в 15:02, должна быть у системы раньше,
     15:03 наступит; если не дождаться, человек успеет закрыть приложение
     в эти сто миллисекунд, и напоминание не придёт вовсе — тихо,
